...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
          sliderInput("mu", label="Mean",min=-4,max=4,value=0,step=0.1),
          sliderInput("sigma2", label="Variance",min=.01,max=10,value=1,step=.1),
          sliderInput("x", label="x",min=-4,max=4,value=0,step=0.1)
        ),
    ...
    output$distPlot <- renderPlot({
      grid.arrange(
        ...
        ggplot()+geom_function(fun=~{pnorm(.x,input$mu,sqrt(input$sigma2))},n=1001)+
          ...
          scale_x_continuous(expand=c(.005,.005),limits=c(-4,4)) + scale_y_continuous(expand=c(.005,0)) + labs(title="Cumulative density function (CDF)",x="x",y="Probability"),
        ggplot()+geom_function(fun=~{qnorm(.x,input$mu,sqrt(input$sigma2))},n=1001)+
          ...
          scale_x_continuous(expand=c(.005,.005),limits=c(0,1)) + scale_y_continuous(expand=c(.005,.005),limits=c(-4,4)) + labs(title="Inverse CDF",x="Probability",y="x"),
